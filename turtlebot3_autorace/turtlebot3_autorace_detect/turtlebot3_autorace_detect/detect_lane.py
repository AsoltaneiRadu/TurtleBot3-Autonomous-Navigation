...
class DetectLane(Node):
    ...
    def __init__(self):
        ...
        # --- NEW: Descriptor for Width/Height (0 to 1000) ---
        parameter_descriptor_size = ParameterDescriptor(
            description='frame size range',
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('detect.lane.white.lightness_l', 85, parameter_descriptor_saturation_lightness),
                ('detect.lane.white.lightness_h', 255, parameter_descriptor_saturation_lightness),
                ('detect.lane.yellow.hue_l', 0, parameter_descriptor_hue),
                ('detect.lane.yellow.hue_h', 179, parameter_descriptor_hue),
                ...
                ('detect.lane.yellow.lightness_l', 85, parameter_descriptor_saturation_lightness),
                ('detect.lane.yellow.lightness_h', 255, parameter_descriptor_saturation_lightness),
                # --- GEOMETRY PARAMETERS ---
                ('detect.lane.x_l', 0, parameter_descriptor_size),
                ('detect.lane.x_r', 640, parameter_descriptor_size),
                ('detect.lane.y_l', 240, parameter_descriptor_size),
                ('detect.lane.y_r', 480, parameter_descriptor_size),
                # ---------------------------
        ...
        self.x_l = self.get_parameter('detect.lane.x_l').get_parameter_value().integer_value
        self.x_r = self.get_parameter('detect.lane.x_r').get_parameter_value().integer_value
        self.y_l = self.get_parameter('detect.lane.y_l').get_parameter_value().integer_value
        self.y_r = self.get_parameter('detect.lane.y_r').get_parameter_value().integer_value
        # --------------------------------
        ...
        if self.sub_image_type == 'compressed':
            ...
            # Folosim qos_profile_sensor_data pentru a accepta "Best Effort" de la Gazebo
            self.sub_image_original = self.create_subscription(Image, '/detect/image_input', self.cbFindLane, qos_profile_sensor_data)
        ...
        if True:
            if self.pub_image_type == 'compressed':
                ...
                self.pub_image_lane_transform = self.create_publisher(Image, '/detect/image_output/lane_transform', 1)
            elif self.pub_image_type == 'raw':
    ...
    def cbGetDetectLaneParam(self, parameters):
        for param in parameters:
            ...
            # --- FIX: UPDATE GEOMETRY PARAMS DYNAMICALLY ---
            elif param.name == 'detect.lane.x_l':
                self.x_l = param.value
            elif param.name == 'detect.lane.x_r':
                self.x_r = param.value
            elif param.name == 'detect.lane.y_l':
                self.y_l = param.value
            elif param.name == 'detect.lane.y_r':
                self.y_r = param.value
            # -----------------------------------------------
        return SetParametersResult(successful=True)

    def birdView(self, img):
        # This function does the math to stretch the image
        img_h, img_w = img.shape[:2]

        src = np.float32([
            [self.x_l, self.y_l],          # Top Left
            [self.x_r, self.y_r],          # Top Right
            [self.x_r, img_h],             # Bottom Right
            [self.x_l, img_h]              # Bottom Left
        ])
        ...
        img_bird = cv2.warpPerspective(img, M, (img_w, img_h))
        return img_bird
    ...
    def cbFindLane(self, image_msg):
        ...
        # --- FIX: CALCULATE AND PUBLISH THE TRANSFORM ---
        # This creates the "Bird's Eye View"
        img_bird = self.birdView(cv_image)
        self.pub_image_lane_transform.publish(self.cvBridge.cv2_to_imgmsg(img_bird, 'bgr8'))
        # ------------------------------------------------

        # Note: The lane detection below typically runs on the TRANSFORMED image,
        # but to keep it simple and safe for now, we will feed it the raw image
        # or the transformed one depending on your preference.
        # For now, let's keep the detection logic as it was (on cv_image)
        # unless you want to detect ON the map.
        ...
        try:
            if yellow_fraction > 3000:
                if self.left_fit is None:
                    raise ValueError("No previous fit")
    ...
    def sliding_window(self, img_w, left_or_right):
        ...
        try:
            ...
            if self.lane_fit_bef is not None:
